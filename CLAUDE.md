...
Currently in `main.ts`: `hardStopAmount: 0.15`, `breakevenActivationAmount: 0.2`, `trailingActivationAmount: 0.3`, `trailingStopAmount: 0.1` (changed from `0.15` on 2026-08-20, alongside the trailing rework below — a deliberate user decision, not derived from the Script Editor source), `quantity: 300`, `minPrice: 1`, `maxPrice: 20`, `maxSpread: 0.03`, `minEntryVolume: 5,000` (env-overridable via `AGENT_MIN_ENTRY_VOLUME`, added 2026-09-03), `maxDailyLossUsd: 150` (raised from `120` on 2026-09-03, see below). Screener thresholds (env-overridable, see `SCREENER_*` vars at the top of `stockScreener.ts`): market cap > $300M, gap ≥ 5% (**gap up only** — the strategy is long-only, so this is a deliberate interpretation, not a given), 10-day avg volume > 1M, relative volume (10d) > 2, float > 20M. If the real Script Editor strategy's inputs ever change, these need to change with it — they are not independently tuned.

**`ACTIVE_SYMBOL_COUNT` raised from 2 to 4, and `maxDailyLossUsd` from 120 to 150, both 2026-09-03.** Motivated by real log evidence: a `daily-reviews/` audit turned up 7 real `LONG_ENTRY` crossovers across 2026-09-01/02 that fired and were then explicitly logged as `[signal] ... ignored — not in top 2` — genuine setups the strategy saw and declined to act on purely because the symbol wasn't ranked in the top-2 watchlist/always-active slots (grep any run's log for `ignored — not in top` to find these). Raising the count to 4 lets more of them through — a deliberate risk-tolerance decision, not a bug fix, since more concurrent active symbols means more simultaneous capital at risk and more correlated exposure if several positions move against the account at once. `maxDailyLossUsd` raised in the same step specifically because a wider active-symbol count raises the realistic worst case for one day's cumulative hard-stop losses; $150 keeps a comparable multiple of headroom above a single `hardStopAmount`-sized loss (`$45` per position at the current `quantity`/`hardStopAmount`) as the original $120 did against the original 2-symbol cap. Not yet verified live — watch the next few sessions for whether more concurrent positions materializes as intended, and whether the wider loss cap is ever actually approached.
